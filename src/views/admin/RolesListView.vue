...
<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('admin.roles.list.title') }}</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ t('admin.roles.list.subtitle') }}</p>
      </div>
      <div v-if="canCreate">
        <button
    ...
    <div v-if="filteredRoles.length === 0" class="flex flex-col items-center justify-center py-16">
      ...
      <p class="text-sm font-semibold text-gray-600 dark:text-gray-300">{{ t('admin.roles.empty.title') }}</p>
      <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ t('admin.roles.empty.subtitle') }}</p>
    </div>
    <Teleport to="body">
      <Transition
        ...
        <div v-if="deleteConfirmId" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-sm w-full p-6 border border-gray-100 dark:border-gray-700">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ t('admin.roles.modal.confirm_delete.title') }}</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ t('admin.roles.modal.confirm_delete.message') }}</p>
              </div>
            </div>
            <div class="flex items-center justify-end gap-3 mt-6">
              ...
              >
                {{ t('admin.roles.modal.confirm_delete.cancel') }}
              </button>
              <button
              ...
              >
                {{ t('admin.roles.modal.confirm_delete.delete') }}
              </button>
            </div>
          </div>
        </div>
