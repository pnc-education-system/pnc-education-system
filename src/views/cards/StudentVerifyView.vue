...
const cardNotice = computed(() => {
  ...
  if (!hasCard.value) {
    return 'This student does not have an ID card yet. The card needs to be generated by an administrator.'
  }
  return ''
})

// Construct photo URL from backend photo_path
const photoUrl = computed(() => {
  if (!student.value?.photo_path) return null
  if (/^https?:\/\//i.test(student.value.photo_path)) return student.value.photo_path
...
// The token can be a QR token (from generated cards) or a student_id_no (from verification links)
async function fetchStudent() {
  loading.value = true
  error.value = ''
  photoError.value = false
  try {
    // First try looking up by QR token (works for generated cards with QR codes)
    try {
      ...
      // QR token lookup failed — try looking up by student_id_no instead
    }

    // Fallback: look up by student ID number (works for verification links)
  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-900 dark:to-gray-800 flex items-start justify-center p-4 sm:p-6">
    <div class="w-full max-w-md">
      ...
      <div v-if="!loading && valid && student" class="bg-white dark:bg-gray-800/80 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div class="px-5 pb-5">
          <!-- Card Notice Banner (when no card generated yet) -->
          <div v-if="cardNotice" class="mx-0 mt-2 mb-4 px-4 py-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 flex items-start gap-3">
            <AlertCircle :size="18" class="text-amber-500 shrink-0 mt-0.5" />
            <div>
              <p class="text-xs font-semibold text-amber-800 dark:text-amber-300">No ID Card Generated</p>
              <p class="text-xs text-amber-600/80 dark:text-amber-400/70 mt-0.5">{{ cardNotice }}</p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3 mb-4">
            ...
            <div class="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700/50">
              ...
              <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ (student as any).selection_batch?.name || student.selection_batch_name || '—' }}</p>
            </div>
            <div class="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700/50">
              <div class="flex items-center gap-1.5 mb-1">
